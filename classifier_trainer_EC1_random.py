...
for i in range(0,len(EC_total1)):
    ...
    EC1_list = []
    if (len(EC_total_struc) > 4):
        for j in range(0,len(EC_total_struc)):
            if EC_total_struc[j].split('.')[0] not in EC1_list:
                EC1_list.append(EC_total_struc[j].split('.')[0])
        print(EC_total_struc)
        print(EC1_list)
        print(i)
        # sum+=1
    if len(EC1_list) > 1:
